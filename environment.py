import gym
import CS231N_Project.preprocess_frame as ppf
import numpy as np
...
def take_step(name, env, agent, score, debug):
    ...
    if agent.total_timesteps % 5000 == 0:
        Q_vals = agent.model.predict(new_state)
        Q_vals = Q_vals.tolist()
        Q_vals = Q_vals[0]
        print('QVALS', Q_vals)
        fig = plt.figure(figsize=(7, 7))
        #plt.imshow(new_state)
        acts = np.array(agent.possible_actions,dtype='str')
        plt.bar(acts, Q_vals, color=['red', 'green', 'cyan'])
        colors = {'keep heading': 'red', 'decrease heading': 'green', 'increase heading': 'cyan'}
